...
class App(ctk.CTk):
    ...
    def __init__(self):
        ...
        self.folder_entry = ctk.CTkEntry(
            ...
            placeholder_text="Render directory:",
            font=self.font,
            width=c.FOLDER_ENTRY_WIDTH,
        )
        ...
        self.folder_btn = ctk.CTkButton(
            ...
            text="Browse",
            font=self.font,
            command=self.folder_btn_callback,
        )
        ...
        self.range_check = ctk.CTkCheckBox(
        ...
        self.range_sframe = ctk.CTkEntry(
            master=self.range_frame,
            placeholder_text="First Frame",
            font=self.font,
            width=c.RANGE_ENTRY_WIDTH,
        )
        self.range_sframe.grid(row=0, column=1, padx=10, pady=5)
        self.range_eframe = ctk.CTkEntry(
            master=self.range_frame,
            placeholder_text="Last Frame",
            font=self.font,
            width=c.RANGE_ENTRY_WIDTH,
        )
        ...
        self.method_opt = ctk.CTkOptionMenu(
            master=self.method_frame, values=c.METHOD_OPTIONS, font=self.font, width=90
        )
        self.method_opt.grid(row=0, column=3, padx=10, pady=5)
        self.method_label = ctk.CTkLabel(
            master=self.method_frame, text="Method", font=self.font
        )
        ...
        self.chan_opt = ctk.CTkOptionMenu(
            master=self.chan_frame, values=c.CHAN_OPTIONS, font=self.font, width=60
        )
        self.chan_opt.grid(row=0, column=5, padx=10, pady=5)
        self.chan_label = ctk.CTkLabel(
            master=self.chan_frame, text="Channel", font=self.font
        )
        ...
        self.inspect_btn = ctk.CTkButton(
            ...
            text="Inspect",
            font=self.font,
            command=self.inspect_btn_callback,
        )
        self.inspect_btn.grid(row=0, column=1, padx=10, pady=5)

        self.copy_missing_btn = ctk.CTkButton(
            master=self.btn_frame, text="To Clipboard", font=self.font
        )
        ...
        self.sequence_textbox = ctk.CTkTextbox(
            master=self.seq_frame,
            font=self.font,
            wrap="none",
            height=c.SEQ_HEIGHT,
            width=c.SEQ_WIDTH,
        ...
        self.log_label = ctk.CTkLabel(
            master=self.bottom_frame, text="Log: ", font=self.font
        )
    ...
    def log_frames(self):
        ...
        self.files = sequence_collector(
            self.folder_entry.get(),
            self.selected_sequence,
            self.range_check.get(),
            range_min,
            range_max,
        )
    ...
    def inspect_btn_callback(self):
        ...
        self.log.insert(
            "0.0",
            f"Inspection completed in: {int(time_diff_min)} minutes and {int(time_diff_sec)} seconds.\nNumber of suspected files: {len(self.suspects)}.\n\n",
        )
        self.log.configure(state="disabled")
    ...
    def on_click(self, event):
        ...
        self.sequence_textbox.tag_config(
            "highlight", background="#7b3b3b", foreground="white"
        )
    ...
    def preflight_check(self):
        ...
        self.log.configure(state="normal")
        selected_line = int(
            self.sequence_textbox.index("insert").split(".", maxsplit=1)[0]
        )
        if selected_line + 1 >= int(
            self.sequence_textbox.index("end").split(".", maxsplit=1)[0]
        ):
            self.log.delete("0.0", "end")
